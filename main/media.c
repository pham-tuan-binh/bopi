...
{
    esp_codec_dev_handle_t record_handle = bsp_codec_microphone_get();
...
{
    esp_codec_dev_handle_t render_device = bsp_codec_speaker_get();
    ...
    esp_codec_dev_set_out_vol(i2s_cfg.play_handle, CONFIG_LK_BOPI_SPEAKER_VOLUME);
...
void media_set_mic_muted(bool muted)
{
    ...
    if (mic != NULL) {
        esp_codec_dev_set_in_mute(mic, muted);
    }
}